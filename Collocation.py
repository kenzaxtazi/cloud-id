...
def collocate(SLSTR_filename, Calipso_filename):
    ...
    def is_matching(slat, slon, clat, clon, indices, tolerance=0.00224577793):
        lattolerance = tolerance
        matches = abs(slat[i, j] - clat) < lattolerance
        if matches.any():
            loc = np.where(matches is True)
            lontolerance = (tolerance / np.cos(slat[i, j] * np.pi / 180))
            for k in loc[0]:
                if abs(slon[i, j] - clon[k]) < lontolerance:
                    return(True)
                    
             
    def findedgepixel(slat, slon, clat, clon):
        ...
        for i in [0, 1, 2, 2399, 2398, 2397]:
            for j in range(3000):
                ...
                if matches.any():
                    ...
                    lontolerance = (0.00224577793 /
                                    np.cos(slat[i, j] * np.pi / 180))
                    for k in loc[0]:
                        if abs(slon[i, j] - clon[k]) < lontolerance:
                            if i in [0, 1, 2]:
                                edge = 'top'
    ...
    coords, edge = findedgepixel(slat, slon, clat, clon)
